...
### 4. apps/web/src/App.tsx — toast manual dismiss (#27, settled half)
- The resolve/feed toast (the `{toast && (...)}` block at the bottom of App.tsx) gains a manual dismiss button as its LAST child: a `<button type="button">` with `aria-label="Dismiss notification"`, visible glyph `×`, `onClick={() => setToast(null)}`, styled quietly (`text-slate-400 hover:text-white text-sm leading-none`). Keep the 4s auto-dismiss effect exactly as-is — manual dismiss is additive.
- Do NOT touch the toast message span, the `✦`/`✦ distilled` span, or the jump "View" button — existing e2e assert those strings as single text nodes and must keep passing unchanged.
- e2e: add one test to e2e/resolve.spec.ts named 'toast can be dismissed manually': open Settings, click 'Reset demo data' (in the drawer per section 2/1), expect the '✓ Demo data reset to the pristine seed' toast visible, click `getByRole('button', { name: 'Dismiss notification' })`, expect the toast text not visible. No timer assertions (auto-dismiss stays untested here).

### 5. e2e updates (use ./fixtures — its '**/distill' abort stays the baseline)
