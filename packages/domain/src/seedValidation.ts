...
export function validateSeedBundle(bundle: SeedBundle): string[] {
  ...
  for (const d of bundle.decisions) validateDecision(d, ids, violations, false);
  for (const d of bundle.feedDecisions ?? []) validateDecision(d, ids, violations, true);
...
function validateDecision(d: Decision, ids: Set<string>, violations: string[], feedDecision: boolean): void {
  ...
  ids.add(d.id);
  if (feedDecision && d.status !== 'open') at('feed decisions must arrive open');
  ...
  e.cases.forEach((c, i) => {
    ...
    if (c.patternIndex !== undefined && (c.patternIndex < 0 || c.patternIndex >= e.patterns.length)) at(`case ${i} patternIndex out of range`);
